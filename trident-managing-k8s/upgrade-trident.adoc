...
* 릴리스 4개가 아닌 다른 릴리스에서 업그레이드하는 경우 여러 단계로 업그레이드를 수행합니다. 의 업그레이드 지침을 사용합니다 link:../earlier-versions.html["이전 버전"] 에서 4개의 릴리즈 윈도우에 맞는 최신 릴리즈로 업그레이드하는 중입니다. 예를 들어 21.10을 실행 중이고 23.10으로 업그레이드하려는 경우:
+

...
다음 단계에 따라 * Trident 21.01 이상에서 업그레이드합니다 *.
...
----
. 필요에 따라 를 편집할 수 있습니다 `TridentOrchestrator` Trident를 설치할 때 생성한 객체를 선택하여 설치 매개 변수를 수정합니다. 여기에는 사용자 지정 Trident 이미지 수정, 컨테이너 이미지를 가져올 전용 이미지 레지스트리, 디버그 로그 활성화 또는 이미지 풀 암호 지정 등의 변경 사항이 포함될 수 있습니다.
. 사용자 환경에 맞는 올바른 번들 YAML 파일과 의 Astra Trident 버전을 사용하여 Astra Trident를 설치합니다 `\https://github.com/NetApp/trident/tree/stable/_vXX.XX_/deploy/_BUNDLE.YAML_` 위치 `_vXX.XX_` 은 버전 번호입니다(예 `v22.10`) 및 `_BUNDLE.YAML_` 번들 YAML 파일 이름입니다.
+
[NOTE]
====
...
====
+
예를 들어, Kubernetes 1.25용 Astra Trident 22.10을 설치하는 경우 다음 명령을 실행합니다.
...
----
. 매니페스트를 사용하여 네임스페이스 범위 연산자를 삭제합니다. 이 단계를 완료하려면 에서 네임스페이스 범위 연산자를 배포하는 데 사용되는 번들 YAML 파일이 필요합니다 `\https://github.com/NetApp/trident/tree/stable/_vXX.XX_/deploy/_BUNDLE.YAML_` 위치 `_vXX.XX_` 은 버전 번호입니다(예 `v22.10`) 및 `_BUNDLE.YAML_` 번들 YAML 파일 이름입니다.
+

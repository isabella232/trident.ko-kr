...
| '네트워크 기능' | 볼륨에 대한 VNET 기능 집합은 기본 또는 표준(standard)일 수 있습니다 | "" 
...
NOTE: 를 클릭합니다 https://docs.microsoft.com/en-us/azure/azure-netapp-files/configure-network-features["네트워크 기능"^] 기능은 일반적으로 일부 지역에서 사용할 수 없으며 구독에서 활성화해야 할 수도 있습니다. 기능이 활성화되지 않은 경우 'networkFeatures' config 옵션을 지정하면 Trident에서 볼륨 프로비저닝이 실패합니다.
...
----
다음 'torageClass' 정의는 위의 스토리지 풀을 참조합니다. parameters.selector` 필드를 사용하면 볼륨을 호스팅하는 데 사용되는 가상 풀을 각 'torageClass'에 지정할 수 있습니다. 볼륨은 선택한 풀에 정의된 측면을 갖습니다.

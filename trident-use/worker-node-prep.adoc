...
드라이버를 함께 사용하는 경우 NFS 및 iSCSI 도구를 설치해야 합니다.
...
NOTE: 최신 버전의 RedHat CoreOS에는 기본적으로 NFS 및 iSCSI가 설치되어 있습니다.
...
----
* iSCSI PVS에서 RHEL/RedHat CoreOS를 실행하는 작업자 노드를 사용하는 경우 를 지정합니다 `discard` StorageClass의 mountOption을 사용하여 인라인 공간 재확보를 수행합니다. 을 참조하십시오 https://["RedHat 설명서"^].

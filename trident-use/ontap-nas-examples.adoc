...
.ONTAP NAS 경제성 최소 구성 예
[%collapsible%open]
====
이것은 를 사용하는 기본 구성입니다 `ontap-nas-economy` 드라이버.
...
----
====
.ONTAP NAS FlexGroup 최소 구성 예
[%collapsible%open]
====
이것은 를 사용하는 기본 구성입니다 `ontap-nas-flexgroup` 드라이버.
...
[%collapsible%open]
====
이 예에서는 Astra Trident가 동적 엑스포트 정책을 사용하여 엑스포트 정책을 자동으로 생성하고 관리하도록 지시합니다. 이 기능은 에 대해서도 동일하게 작동합니다 `ontap-nas-economy` 및 `ontap-nas-flexgroup` 드라이버.
...
[%collapsible%open]
====
이 예에서는 SMB 볼륨을 사용하여 ONTAP 백엔드에 대한 Amazon FSx를 생성합니다.

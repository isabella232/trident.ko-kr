...
NFS 마운트 옵션을 세밀하게 제어합니다. 기본값은 "-o nfsvers=3"입니다. * ONTAP-NAS 및 ONTAP-NAS-이코노미 드라이버에만 사용 가능 *. https://["NFS 호스트 구성 정보는 여기 를 참조하십시오"^].
 a| 
`-o nfsvers=4`
...
FlexGroups에 적합한 FlexGroups 및 워크로드에 대한 자세한 내용은 를 참조하십시오 https://["NetApp FlexGroup 볼륨 모범 사례 및 구현 가이드 를 참조하십시오"^].

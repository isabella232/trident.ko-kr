...
인치 `operator.yaml`, 업데이트 `image: docker.io/netapp/trident-operator:23.01.0` 이미지 레지스트리의 위치를 반영합니다. 예:.

* `image: <your-registry>/trident-operator:23.01.0` 이미지가 모두 하나의 레지스트리에 있는 경우
* `image: <your-registry>/netapp/trident-operator:23.01.0` Trident 및 CSI 이미지가 다른 레지스트리에 있는 경우
...
=== 4단계: 에서 이미지 레지스트리 위치를 업데이트합니다 `TridentOrchestrator`

Trident 및 CSI 이미지는 하나의 레지스트리 또는 다른 레지스트리에 위치할 수 있지만 모든 CSI 이미지는 동일한 레지스트리에 있어야 합니다. 업데이트 `deploy/crds/tridentorchestrator_cr.yaml` 레지스트리 구성에 따라 추가 위치 사양을 추가하려면 다음을 수행합니다.

[role="tabbed-block"]
====
.하나의 레지스트리에 Trident 및 CSI 이미지
--
[listing]
----
...
=== 5단계: 을 작성합니다 `TridentOrchestrator` Trident를 설치합니다
